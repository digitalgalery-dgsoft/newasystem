...
<div x-data="{ 
    ...
    <div x-show="archiveModal" 
        ...
        <div @click.away="archiveModal = false" 
            ...
            <form action="{{ route('interview.archive', $candidate->id) }}" method="POST" class="space-y-4">
                ...
                <p class="text-xs text-slate-600">
                    Kandidat <b>{{ $candidate->full_name }}</b> akan dipindahkan ke daftar arsip interview.
                </p>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1.5">Alasan Pengarsipan</label>
                    <textarea name="archive_reason" rows="3" class="w-full bg-white border border-slate-200 rounded-xl p-3 text-xs text-slate-800 focus:ring-4 focus:ring-rose-100 focus:border-rose-600 outline-none" placeholder="Masukkan alasan pengarsipan (misal: Tidak hadir, Kualifikasi belum sesuai, Batal ikut interview)..."></textarea>
                </div>
        ...
        $principleSelectOptions = collect($principles ?? [])->map(function($p) {
            return [
                'value' => (string)$p->id,
                'label' => $p->name,
                'sublabel' => $p->code ? 'Kode: ' . $p->code : ''
            ];
        })->values()->all();
        ...
        if (!empty($candidate->area) && !in_array($candidate->area, $areaList)) {
            $areaList[] = $candidate->area;
        }
        $areaSelectOptions = collect($areaList)->map(function($ar) {
            return [
                'value' => $ar,
                'label' => $ar,
                'sublabel' => ''
        ...
        } elseif ($candidate->principle && is_object($candidate->principle) && !empty($candidate->principle->id)) {
            $selectedPrincipleId = (string)$candidate->principle->id;
        } elseif (is_string($candidate->principle) && !empty($candidate->principle)) {
            $matched = collect($principles ?? [])->first(function($p) use ($candidate) {
                return strtolower(trim($p->name)) === strtolower(trim($candidate->principle));
            });
            if ($matched) {
                $selectedPrincipleId = (string)$matched->id;
            }
        }
        if (empty($selectedPrincipleId) && !empty($principles) && count($principles) > 0) {
            $selectedPrincipleId = (string)($principles[0]->id ?? '');
        }
    @endphp
    ...
    <div x-show="alihkanModalOpen" 
        ...
        <div @click.away="alihkanModalOpen = false" 
            ...
            <div class="flex items-center justify-between pb-3 border-b border-slate-100">
                ...
                <button @click="alihkanModalOpen = false" class="text-slate-400 hover:text-slate-600 cursor-pointer">
                    <i class="fa-solid fa-xmark text-base"></i>
                </button>
            </div>
            ...
            <form action="{{ route('interview.alihkan', $candidate->id) }}" method="POST" class="space-y-4">
                ...
                <!-- User AS / Rekruter Tujuan (Searchable Dropdown) -->
                ...
                <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                    <button @click="alihkanModalOpen = false" type="button" class="px-4 py-2 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-100 cursor-pointer">
                        Batal
                    </button>
                    <button type="submit" class="px-5 py-2 rounded-xl text-xs font-bold bg-primary hover:bg-primary-700 text-white shadow-sm cursor-pointer transition">
                        Simpan & Alihkan
                    </button>
                </div>
            </form>
    ...
    <div x-show="gantiAreaModalOpen" 
        ...
        <div @click.away="gantiAreaModalOpen = false" 
            ...
            <div class="flex items-center justify-between pb-3 border-b border-slate-100">
                ...
                <button @click="gantiAreaModalOpen = false" class="text-slate-400 hover:text-slate-600 cursor-pointer">
                    <i class="fa-solid fa-xmark text-base"></i>
                </button>
            </div>
