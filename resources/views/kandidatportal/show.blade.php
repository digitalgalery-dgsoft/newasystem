...
<div class="space-y-6" x-data="kandidatDetailManager()">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <!-- Right: Foto Profil 3x4 Dropzone -->
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden" x-data="{ activeTab: 'interview' }">
        ...
        <div class="border-b border-slate-200 bg-slate-50/70 px-4 sm:px-6 pt-3 flex items-center gap-2 overflow-x-auto">
            ...
            <!-- 4. Analisa AI (Special Job Portal Tab!) -->
            <button @click="activeTab = 'ai'" 
                ...
                <span>4. Analisa AI (CV Analyzer)</span>
            ...
            <button @click="activeTab = 'kepribadian'" 
                ...
                <span>5. Tes Kepribadian (DISC)</span>
            </button>
        ...
        <div class="p-6">
            ...
            <div x-show="activeTab === 'refcek'" class="space-y-6">
                @php
                    $exp = $candidate->workExperiences->first();
                @endphp
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-3 text-xs">
                            <div>
                                <label class="block text-slate-500 font-medium mb-1">Perusahaan Sebelumnya</label>
                                <input type="text" value="{{ $exp ? $exp->company_name : '-' }}" readonly class="w-full px-3 py-2 rounded-xl border border-slate-200 bg-slate-50 font-semibold text-slate-800">
                            </div>

                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-slate-500 font-medium mb-1">Nama SPV / Referensi</label>
                                    <input type="text" value="{{ $exp ? $exp->supervisor_name : '-' }}" class="w-full px-3 py-2 rounded-xl border border-slate-300 font-medium text-slate-800">
                                </div>
                                <div>
                                    <label class="block text-slate-500 font-medium mb-1">Telp Perusahaan / SPV</label>
                                    <input type="text" value="{{ $exp ? $exp->company_phone : '-' }}" class="w-full px-3 py-2 rounded-xl border border-slate-300 font-medium text-slate-800">
                                </div>
                            </div>

                            <div class="grid grid-cols-3 gap-2">
                                <div>
                                    <label class="block text-slate-500 font-medium mb-1">Performa</label>
                                    <input type="text" value="{{ $exp ? $exp->performance_review : 'Baik' }}" class="w-full px-2.5 py-1.5 rounded-lg border border-slate-300 text-xs">
                                </div>
                                <div>
                                    <label class="block text-slate-500 font-medium mb-1">Disiplin</label>
                                    <input type="text" value="{{ $exp ? $exp->discipline_review : 'Tepat Waktu' }}" class="w-full px-2.5 py-1.5 rounded-lg border border-slate-300 text-xs">
                                </div>
                                <div>
                                    <label class="block text-slate-500 font-medium mb-1">Tanggung Jawab</label>
                                    <input type="text" value="{{ $exp ? $exp->responsibility_review : 'Bertanggung Jawab' }}" class="w-full px-2.5 py-1.5 rounded-lg border border-slate-300 text-xs">
                                </div>
                            </div>

                            <div>
                                <label class="block text-slate-500 font-medium mb-1">Keunggulan Utama (Strengths)</label>
                                <textarea rows="2" class="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs text-slate-800">{{ $exp ? $exp->strengths_identified : 'Problem solving cepat dan komunikatif.' }}</textarea>
                            </div>

                            <div>
                                <label class="block text-slate-500 font-medium mb-1">Kelemahan (Weaknesses)</label>
                                <textarea rows="2" class="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs text-slate-800">{{ $exp ? $exp->weaknesses_identified : 'Terkadang terlalu detail.' }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="p-6 rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 text-center flex flex-col items-center justify-center hover:border-emerald-400 transition-all cursor-pointer">
                            ...
                            <button type="button" class="mt-3 px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200 text-xs font-bold hover:bg-emerald-100">
                                Pilih File Screenshot
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div x-show="activeTab === 'komputer'" class="space-y-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-[11px] text-slate-500 mt-0.5">Waktu Pengerjaan: <span class="font-bold text-slate-800">00:03:10</span> • Skor Penguasaan: <span class="font-bold text-emerald-600">90% (Sangat Baik)</span></p>
                    </div>
                </div>
                    ...
                    <table class="w-full text-xs custom-table">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center w-32">Baik</th>
                                <th class="text-center w-32">Cukup</th>
                                <th class="text-center w-32">Kurang</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @php
                                $skills = [
                                    ...
                                    'FUNGSI IF' => 'Baik',
                                    'AVERAGE' => 'Baik',
                                    'PERKALIAN & PEMBAGIAN' => 'Baik',
                                    'KETELITIAN' => 'Baik',
                                    'KECEPATAN' => 'Baik',
                                    'HASIL KERJA' => 'Baik',
                                ];
                            @endphp
                            @foreach($skills as $sk => $val)
                            <tr>
                                <td class="font-bold text-slate-800">{{ $sk }}</td>
                                @foreach(['Baik', 'Cukup', 'Kurang'] as $v)
                                <td class="text-center">
                                    <input type="radio" name="comp_{{ strtolower($sk) }}" value="{{ $v }}" {{ $val === $v ? 'checked' : '' }} class="w-4 h-4 text-primary">
                                </td>
                                @endforeach
                            </tr>
                            @endforeach
            ...
            <!-- ============================================== -->
            <!-- TAB 4: ANALISA AI (CV ANALYZER RESULT)         -->
            <!-- ============================================== -->
            <div x-show="activeTab === 'ai'" class="space-y-6">
                @php
                    $matchScore = intval($aiData['evaluation_match_score'] ?? ($candidate->ai_score ?? 85));
                @endphp
                <div class="p-4 bg-gradient-to-r from-blue-50/70 via-indigo-50/50 to-slate-50 rounded-2xl border border-blue-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        <div class="w-11 h-11 rounded-2xl bg-gradient-to-tr from-primary to-indigo-600 text-white flex items-center justify-center text-xl shadow-md shadow-primary-500/20">
                            <i class="fa-solid fa-wand-magic-sparkles"></i>
                        </div>
                        <div>
                            <h3 class="text-sm font-extrabold text-slate-900">AI CV Analyzer Recommendation</h3>
                            <p class="text-[11px] text-slate-500">Hasil evaluasi otomatis kesesuaian profil kandidat dengan requirement lowongan</p>
                        </div>
                    </div>

                    <div class="flex items-center gap-2">
                        <button type="button" @click="alert('Memulai evaluasi ulang berkas CV kandidat...')" class="px-3.5 py-1.5 rounded-xl bg-white border border-slate-300 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-all shadow-sm">
                            <i class="fa-solid fa-arrows-rotate mr-1 text-primary"></i> Analisis Ulang CV
                        </button>
                    </div>
                </div>

                <!-- Match Radial & Summary Grid -->
                <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
                    ...
                    <div class="md:col-span-4 bg-slate-50 rounded-2xl border border-slate-200 p-6 flex flex-col items-center justify-center text-center">
                        ...
                        <div class="relative w-36 h-36 rounded-full flex items-center justify-center shadow-sm"
                             style="background: conic-gradient({{ $candidate->ai_score_color_hex }} {{ $matchScore }}%, #e2e8f0 0);">
                            <div class="w-28 h-28 bg-white rounded-full flex flex-col items-center justify-center shadow-inner">
                                ...
                                <span class="text-[10px] font-extrabold uppercase mt-1" style="color: {{ $candidate->ai_score_color_hex }}">
                                    {{ $aiData['specification_fit'] ?? 'Fit' }}
                                </span>
                            </div>
                        </div>
                    ...
                    <!-- Details & Skills Breakdown -->
                    <div class="md:col-span-8 bg-white rounded-2xl border border-slate-200 p-6 space-y-4">
                        <div>
                            <h5 class="text-xs font-bold text-slate-900 uppercase tracking-wider mb-2 flex items-center gap-2">
                                <i class="fa-solid fa-circle-check text-emerald-600"></i>
                                <span>Keahlian yang Cocok (Matched Skills)</span>
                            </h5>
                            <div class="flex flex-wrap gap-1.5">
                                @foreach($aiData['matched_skills'] ?? ['PHP', 'Laravel', 'MySQL', 'REST API', 'Git', 'Problem Solving'] as $sk)
                        ...
                        <div>
                            <h5 class="text-xs font-bold text-slate-900 uppercase tracking-wider mb-2 flex items-center gap-2">
                                ...
                                <span>Keahlian yang Perlu Ditingkatkan / Ditanyakan</span>
                            </h5>
                            <div class="flex flex-wrap gap-1.5">
                                @foreach($aiData['missing_skills'] ?? ['Pengalaman Docker Container (Optional)', 'Arsitektur Microservices Lanjutan'] as $ms)
                                <span class="px-2.5 py-1 rounded-lg text-xs font-medium bg-amber-50 text-amber-800 border border-amber-200">
                                    • {{ $ms }}
                                </span>
                                @endforeach
                            </div>
                        ...
                        <div class="pt-3 border-t border-slate-100">
                            <h5 class="text-xs font-bold text-slate-900 uppercase tracking-wider mb-1 flex items-center gap-2">
                                <i class="fa-solid fa-quote-left text-primary"></i>
                                <span>Kesimpulan Rekomendasi AI</span>
                            </h5>
                            <p class="text-xs text-slate-700 leading-relaxed italic bg-slate-50 p-3 rounded-xl border border-slate-200">
                                "{{ $aiData['ai_verdict'] ?? 'Kandidat memiliki rekam jejak kerja yang relevan, keahlian teknis kuat, dan attitude yang sesuai untuk tahapan interview lanjutan.' }}"
                            </p>
                        </div>
                    </div>
                </div>
